class Item < ApplicationRecord
  ...
  def self.unchecked_by_tag_name(tag_name, given_trip)
    select('items.*, checklists.id AS checklist_id, checklists.checked AS checklist_status').tagged_with(tag_name).joins(unchecked: [:trip]).where(checklists: { trip: given_trip })
  end
